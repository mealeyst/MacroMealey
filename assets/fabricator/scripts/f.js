...
/***/ function(module, exports, __webpack_require__) {
	...
	fabricator.buildTypographies = function () {
	
	  var typographies = document.querySelectorAll('.f-typography');
	
	  for (var i = typographies.length - 1; i >= 0; i--) {
	    var fontNode = typographies[i].querySelector('.f-font-family');
	    var font = fontNode.innerHTML;
	    fontNode.remove();
	    var weightNodes = typographies[i].querySelectorAll('.f-font-weight');
	    var weights = [];
	    for (var weight = 0; weight <= weightNodes.length - 1; weight++) {
	      weightNodes[weight].remove();
	      weights.push(weightNodes[weight].innerHTML);
	      var typographyExample = document.createElement('div');
	      typographyExample.className = 'f-typography__example';
	      typographyExample.style.fontFamily = font;
	      typographyExample.style.fontWeight = weights[weight];
	      typographyExample.innerHTML = fontNode.outerHTML + ' ' + weightNodes[weight].outerHTML + ': <span>Sphinx of black quartz, judge my vow!</span>';
	      typographies[i].appendChild(typographyExample);
	    }
	  }
	  return fabricator;
	};
	 ...
	 * Initialization
	 */
	fabricator.setInitialMenuState().menuToggle().allItemsToggles().singleItemToggle().buildColorChips().buildTypographies().setActiveItem().bindCodeAutoSelect();
