...
/***/ function(module, exports) {
	...
	*/
	...
	function modifyToggle(elements, type) {
	  Object.keys(elements).map(function (element) {
	    if (elements[element].id !== 'site-nav-toggle') {
	      ...
	      loadSVG(window.location.origin + '/assets/toolkit/images/' + type + '.svg').then(function (svg) {
	...
	function addIcons() {
	  loadSVG(window.location.origin + '/assets/toolkit/images/spritesheet.svg').then(function (svg) {
	    var parser = new DOMParser();
	    var doc = parser.parseFromString(svg, 'image/svg+xml');
