...
fabricator.buildTypographies = () => {

  const typographies = document.querySelectorAll('.f-typography');

  for (let i = typographies.length - 1; i >= 0; i--) {
    const fontNode = typographies[i].querySelector('.f-font-family');
    const font = fontNode.innerHTML;
    fontNode.remove();
    const weightNodes = typographies[i].querySelectorAll('.f-font-weight')
    const weights = [];
    for (let weight = 0; weight <= weightNodes.length -1; weight++) {
      weightNodes[weight].remove();
      weights.push(weightNodes[weight].innerHTML);
      const typographyExample = document.createElement('div');
